...
import { brandRepo } from "@/lib/repositories";
...
export default async function BrandCatalogPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold tracking-tight text-white">{brand.name} Phones</h1>
        <p className="mt-2 text-sm text-zinc-400">
          Showing {result.total} quality refurbished {brand.name} devices in stock.
        </p>
      </div>
      ...
      {result.items.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-zinc-800 bg-zinc-950 py-20 text-center px-4">
          ...
          <p className="mt-2 max-w-sm text-sm text-zinc-600">
            We don't have any available {brand.name} phones in stock right now. Please check back later or browse other brands.
          </p>
          <Link
            href="/phones"
